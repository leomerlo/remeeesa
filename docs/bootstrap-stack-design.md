...
4. **Supabase project + client wrapper** — manual project creation, `.env`/`.env.example`, `src/lib/supabaseClient.ts` exporting a typed, mockable client.
5. **Supabase Auth providers** — manual: enable email/password, create a Google OAuth client in GCP console, register the redirect URI, enable the Google provider in Supabase. No app UI yet.
...
4. [Manual] Create Supabase project and enable Auth providers
5. Add typed, mockable Supabase client wrapper

...
`cuentas-pendientes.md` (delete/merge must also check Cuentas).
...
every referencing Expense/Cuenta → delete the old doc. Collision check is simply "does a doc
...
- `allow delete`: replace `if false` with a household-membership check. Firestore rules can't
  cheaply express "no Expense/Cuenta references this category" (needs a collection query, not a
...
- **Cross-story requirement:** `cuentas-pendientes.md`'s rules need a narrow extra allowance —
  `category_id`-only updates on a Cuenta regardless of `status`, so merge can repoint paid
  Cuentas too. This is an explicit, intentional exception to "a paid Cuenta can't be edited";
...
3. Category delete with reference guard (checks Expenses **and** Cuentas)
4. Category merge (reuses Slice 2's repoint helper; extends `cuentas` rules per above)

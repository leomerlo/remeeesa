...
# Cuenta (Bill) is a separate concept from Expense, not a status on Expense
...
Decision: **Cuenta** (Bill) is a distinct entity — an obligation to pay, with a category, an
optional expected amount, a due date, and a pending/paid status. Marking a Cuenta paid records
...
Recurrence (a Cuenta repeating monthly, e.g. "Internet") stays a property of Cuenta, resolved at

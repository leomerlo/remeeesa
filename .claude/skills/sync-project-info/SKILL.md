...
description: Re-scan the actual codebase (e.g. after an existing app was dropped into this template) and refresh AGENTS.md's project/stack info to match reality. Use when the user says an app was copied/imported in, AGENTS.md is stale or wrong, or asks to sync/refresh/update the project info or detected stack.
---

# Sync Project Info

Reconcile AGENTS.md's "# Project" section with what's actually in the repo, instead of interviewing from scratch. Use this when real code already exists (a project was copied in, or has drifted from what AGENTS.md says) — most of the answer is derivable from files, not the user's memory.
...
2. **Diff against AGENTS.md.** Read the current AGENTS.md. For each fact above, classify as: matches / stale (contradicts the repo) / missing.
...
4. **Write the update.** Edit AGENTS.md in place:
   ...
   - Preserve everything else in AGENTS.md untouched (principles, skills/agents lists, etc.) unless it's now factually wrong.

## Done when

AGENTS.md's Description/Technologies/Commands/Testing sections match the actual repo and the user has confirmed anything you couldn't derive.
